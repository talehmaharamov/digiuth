...
@section('title', __('third.register_as_teacher'))

@section('content')
    <section class="form-section">
        <form action="{{route('register.teacher')}}" method="post" class="digiuth_form" id="teacher-register"
              enctype="multipart/form-data">

            @csrf
            <div class="login-btn">
                <a class="btn-full" href="/login">{{ __('third.login') }}</a>
            </div>
            ...
            <div class="form-group">
                @if(\Illuminate\Support\Facades\Lang::has('third.fullname_teacher'))
                    <label for="fullName">{{ __('third.fullname_teacher') }}<span>*</span></label>
                @endif
                <input class="form-control" aria-describedby="emailHelp" placeholder="{{__('third.fullname_teacher')}}"
                       type="text"
            ...
            <div class="form-group">
                ...
                @endif
                <input class="form-control" aria-describedby="emailHelp" placeholder="{{__('third.teacher_username')}}"
                       type="text"
                       id="userName"
                       maxlength="50"
                       name="username"
            ...
            <div class="form-group">
                @if(\Illuminate\Support\Facades\Lang::has('third.teacher_password'))
                    <label for="password">{{ __('third.teacher_password') }}<span>*</span></label>
                @endif
                <input class="form-control" type="password" name="password" id="password"
                       placeholder="{{__('third.teacher_password')}}">
            ...
            <div class="form-group">
                @if(\Illuminate\Support\Facades\Lang::has('third.teacher_repassword'))
                    <label for="repassword">{{ __('third.teacher_repassword') }}<span>*</span></label>
                @endif
                <input class="form-control input2" type="password" name="password_confirmation" id="repassword"
                       placeholder="{{__('third.teacher_repassword')}}">
            </div>
