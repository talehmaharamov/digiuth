...
@section('content')
    <section class="form-section">
        <form class="digiuth_form" id="student-register" method="post" action="{{route('register.student')}}">
            ...
            @csrf
            @if(\Illuminate\Support\Facades\Lang::has('third.login'))
                <div class="login-btn">
            ...
            <div class="form-group">
                @if(\Illuminate\Support\Facades\Lang::has('third.your_email'))
                    ...
                    <label for="email">{{ __('third.your_email') }}<span>*</span></label>
                @endif
            ...
            <div class="form-group">
                @if(\Illuminate\Support\Facades\Lang::has('third.student_username'))
                    <label for="userName">{{ __('third.student_username') }}<span>*</span></label>
                @endif
                <input class="form-control" aria-describedby="emailHelp" placeholder="{{__('third.student_username')}}"
                       type="text"
            ...
            <div class="form-group">
                @if(\Illuminate\Support\Facades\Lang::has('third.student_password'))
                    <label for="password">{{ __('third.student_password') }}<span>*</span></label>
                @endif
                <input class="form-control" type="password" name="password" id="password"
                       placeholder="{{__('third.student_password')}}">
            ...
            <div class="form-group">
                @if(\Illuminate\Support\Facades\Lang::has('third.student_repassword'))
                    <label for="repassword">{{ __('third.student_repassword') }}<span>*</span></label>
                @endif
                <input class="form-control input2" type="password" name="password_confirmation" id="repassword"
                       placeholder="{{__('third.student_repassword')}}">
            </div>
